...
const quizAnswers = {};
const quizQuestions = [
  { key: 'visibility', question: 'About how many people regularly notice what you do?', options: [['0–100', 0], ['100–500', .25], ['500–2k', .5], ['2k–10k', .75], ['10k+', 1]] },
  { key: 'outreach', question: 'About how many people do you keep up with?', options: [['0–100', 0], ['100–300', .25], ['300–700', .5], ['700–1.5k', .75], ['1.5k+', 1]] },
  { key: 'connector', question: 'How many different groups do you regularly move between?', options: [['One', .1], ['Two', .35], ['Three or four', .7], ['Five or more', 1]] },
  ...
  { key: 'activity', question: 'How many people do you regularly share space with?', options: [['I live alone', 0], ['One other person', .25], ['Two or three', .55], ['Four or five', .8], ['Six or more', 1]] }
];
...
function twinProfiles() { const degrees = degreeMap(); const neighbors = new Map(storyData.nodes.map((node) => [node.id, new Set()])); storyData.real.edges.forEach(([source, target]) => { neighbors.get(source).add(target); neighbors.get(target).add(source); }); const values = storyData.nodes.map((node) => { const nearby = [...neighbors.get(node.id)]; const links = nearby.reduce((count, left, index) => count + nearby.slice(index + 1).filter((right) => storyData.real.edges.some(([source, target]) => (source === left && target === right) || (source === right && target === left))).length, 0); const clustering = nearby.length > 1 ? links / (nearby.length * (nearby.length - 1) / 2) : 0; return { node, degree: degrees[node.id], visibility: degrees[node.id], outreach: nearby.length, connector: (1 - clustering) * Math.sqrt(degrees[node.id]), closeness: clustering, activity: degrees[node.id] }; }); const percentile = (items) => { const sorted = [...items].sort((a, b) => a - b); return (value) => sorted.indexOf(value) / Math.max(1, sorted.length - 1); }; const keys = ['visibility', 'outreach', 'connector', 'closeness', 'activity']; const ranks = Object.fromEntries(keys.map((key) => [key, percentile(values.map((profile) => profile[key]))])); return values.map((profile) => ({ ...profile, ...Object.fromEntries(keys.map((key) => [key, ranks[key](profile[key])])) })); }
function renderTwinQuiz() { const question = quizQuestions[quizIndex]; const value = quizAnswers[quizIndex]; byId('quiz-step').textContent = `${String(quizIndex + 1).padStart(2, '0')} / ${String(quizQuestions.length).padStart(2, '0')}`; byId('quiz-progress-bar').style.width = `${(quizIndex + 1) / quizQuestions.length * 100}%`; byId('quiz-question').innerHTML = `<legend>${question.question}</legend><div class="quiz-options">${question.options.map(([label, optionValue]) => `<label class="quiz-option"><input type="radio" name="answer" value="${optionValue}" ${value === optionValue ? 'checked' : ''}><span>${label}</span></label>`).join('')}</div>`; byId('quiz-back').disabled = quizIndex === 0; const next = byId('quiz-next'); next.disabled = value === undefined; next.textContent = quizIndex === quizQuestions.length - 1 ? 'Find my twin →' : 'Next →'; }
function renderTwinResult() { const targets = {}; quizQuestions.forEach((question, index) => { targets[question.key] = targets[question.key] === undefined ? quizAnswers[index] : (targets[question.key] + quizAnswers[index]) / 2; }); const keys = ['visibility', 'outreach', 'connector', 'closeness', 'activity']; const best = twinProfiles().reduce((winner, profile) => { const distance = keys.reduce((sum, key) => sum + (profile[key] - targets[key]) ** 2, 0); const winnerDistance = keys.reduce((sum, key) => sum + (winner[key] - targets[key]) ** 2, 0); return distance < winnerDistance ? profile : winner; }); const neighbors = new Set(); storyData.real.edges.forEach(([source, target]) => { if (source === best.node.id) neighbors.add(target); if (target === best.node.id) neighbors.add(source); }); const neighborAverage = [...neighbors].reduce((sum, id) => sum + degreeMap()[id], 0) / Math.max(1, neighbors.size); const role = best.connector > .75 ? 'a natural bridge between different rooms' : best.closeness > .75 ? 'at home in a close-knit circle' : best.visibility > best.outreach ? 'a name many others gravitate toward' : 'a curious connector who reaches outward'; byId('footprint-quiz').hidden = true; const result = byId('quiz-result'); result.hidden = false; result.innerHTML = `<p class="result-kicker">Your Marvel network twin</p><h3 class="result-name">${best.node.name}</h3><p class="result-copy">Your answers most resemble ${best.node.name}: ${role}. Their network degree is <strong>${best.degree}</strong>, and their neighbors average <strong>${format(neighborAverage)}</strong> connections.</p><div class="result-stats"><div><strong>${best.degree}</strong><span>network connections</span></div><div><strong>${neighbors.size}</strong><span>characters nearby</span></div><div><strong>${format(neighborAverage)}</strong><span>average neighbor degree</span></div></div><button class="quiz-restart" type="button" id="quiz-restart">Try another footprint ↻</button>`; byId('quiz-restart').addEventListener('click', () => { Object.keys(quizAnswers).forEach((key) => delete quizAnswers[key]); quizIndex = 0; result.hidden = true; byId('footprint-quiz').hidden = false; renderTwinQuiz(); }); }
function initTwinQuiz() { byId('quiz-form').addEventListener('change', (event) => { quizAnswers[quizIndex] = Number(event.target.value); byId('quiz-next').disabled = false; }); byId('quiz-back').addEventListener('click', () => { quizIndex -= 1; renderTwinQuiz(); }); byId('quiz-form').addEventListener('submit', (event) => { event.preventDefault(); if (quizAnswers[quizIndex] === undefined) return; if (quizIndex === quizQuestions.length - 1) renderTwinResult(); else { quizIndex += 1; renderTwinQuiz(); } }); renderTwinQuiz(); }

async function init() { storyData = await fetch(DATA_URL).then((response) => response.json()); renderStats(); const ids = storyData.nodes.map((node) => node.id); drawNetwork('#real-network', storyData.real.edges, ids, { labels: true }); updateShuffle(); renderNullMetrics(); renderBuild(); renderCcdf(); renderFriends(); initTwinQuiz(); byId('shuffle-slider').addEventListener('input', updateShuffle); byId('subset-select').addEventListener('change', updateShuffle); byId('grow-button').addEventListener('click', grow); document.querySelectorAll('[data-scroll-to]').forEach((button) => button.addEventListener('click', () => byId(button.dataset.scrollTo).scrollIntoView({ behavior: 'smooth' }))); window.addEventListener('resize', () => { drawNetwork('#real-network', storyData.real.edges, ids, { labels: true, animate: false }); updateShuffle(); renderCcdf(); }); }
