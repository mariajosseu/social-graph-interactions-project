...
class El {constructor(tag='div'){this.tag=tag;this.children=[];this.value='';this.attrs={};} append(...items){this.children.push(...items);} replaceChildren(...items){this.children=items;if(this.tag==='select'&&items.length)this.value=String(items[0].value);} setAttribute(key,value){this.attrs[key]=value;} addEventListener(key,fn){this.events ||= {};this.events[key]=fn;} querySelector(){return new El();} getBoundingClientRect(){return {bottom:100,right:600};}}
const ids={};
...
vm.runInContext(source.slice(0,source.indexOf('init().catch')),ctx);
(async()=>{
 ...
 for(const mode of ['undirected','directed','undirected']){
  ...
  assert(ids['homophily-coverage'].innerHTML.includes('Character coverage'));
 }
